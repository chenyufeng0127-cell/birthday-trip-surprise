...
> 🌐 **在线试用（不用下载）**：[示例成品页](https://chenyufeng0127-cell.github.io/birthday-trip-surprise/) · [在线制作向导](https://chenyufeng0127-cell.github.io/birthday-trip-surprise/builder/)
...
- ✅ 背景音乐：内置实时合成旋律 + 5 首自研主题 BGM，也可上传自己的音乐或填外链，按页面 / 站点 / 风格分配
...
默认每个页面与站点都用**浏览器实时合成的旋律**（零文件、零体积）。想更有味道，可以换成你自己的音乐：

| 音源 | 怎么来 | 成品里的表现 |
| --- | --- | --- |
| 内置合成旋律 | 向导里直接选（16 种情绪主题） | 实时合成，不占体积 |
| **内置 BGM** | 向导里选「海边暖沙 / 森林 / 星光夜 / 新婚燕尔 / 圣诞颂歌」 | 自研免版权曲目，内嵌进成品（每首约 235KB） |
| **上传自己的音乐** | 上传 mp3 / m4a / wav（≤15MB） | 内嵌进成品，离线可听 |
| **外链地址** | 填一个 https 音频直链 | 以网址保留，成品打开时需联网 |

分配粒度有三层，逐层回退：**站点级**（每个站点单独指定）→ **页面级**（封面 / 地图 / 回忆册 / 终章各一个）→ **主题级**（按当前风格配套）→ 内置默认。选风格时若主题级还没配，会自动配上该风格的内置 BGM。

两个小贴士：一首 3 分钟的 mp3 内嵌后约 4MB，会让成品明显变大——背景音乐建议剪到 **1 分钟以内**（循环播放）或压到 128kbps；另外请使用**你有权使用**的音乐（自制 / 免版权 / 已获授权），因为导出成品会把音乐一起分发出去。

> ⚠️ **关于仓库里内置的背景音乐**：它们来自 [Pixabay](https://pixabay.com/zh/music/)（免费、可商用、免署名），在本项目里**仅作为配套素材**用于向导试听与成品内嵌。Pixabay 许可**不允许把素材本身以独立文件形式再分发或出售**——所以**请不要把这几个 mp3 单独提取出来做素材库、二次分发或出售**；想要同款音乐请到 Pixabay 自行下载（向导的音乐卡片里有各主题的直达搜索链接）。来源与许可见 [`docs/CREDITS.md`](docs/CREDITS.md)。
...
├── assets/
│   ├── ai/ icons/ map/ avatars/ music/ photos/sample/  # 内置素材（插画/图标/地图/小人/BGM/示例照片，均可替换）
...
├── scripts/make-bgm.py                            # 重新生成内置背景音乐（自研，免版权）
